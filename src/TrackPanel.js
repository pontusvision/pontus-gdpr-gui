...
// import {iosPricetagsOutline} from 'react-icons-kit/ionicons/iosPricetagsOutline';
// import {download} from 'react-icons-kit/entypo/download';
// import { ic_child_care } from 'react-icons-kit/md/ic_child_care';
...
class TrackPanel extends Component
{
  ...
  render()
  {
    ...
    return (
      ...
      <ResizeAware onResize={this.handleResize} height={'100%'}>
        <NavPane openLength={200} push color={this.props.color} height={'100%'}
                 theme={this.props.theme} ref={this.setNode}>
          {this.renderItem("Simple", <NavPanelTrackSimple/>, <Icon icon={child}/>)}
          {this.renderItem("Expert", <NavPanelTrackExpert/>, <Icon icon={lab}/>)}
  ...
  renderItem(title, content, icon)
  {
    ...
    return (
      <NavPaneItem
        title={title}
        icon={icon}
        style={{ maxHeight: this.state.height, height: this.state.height, width: '100%'}}
        theme="light"
        background="#000000"
        selected={this.state.selected === title}
        onSelect={() => this.setState({selected: title})}
