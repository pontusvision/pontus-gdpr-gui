...
class NavPanelAwarenessPopup extends PVGDPRScores
{
  ...
  getSearchQuery = () =>
  {
    ...
    return {
      gremlin: "long numEvents = g.V().has('Metadata.Type','Object.Awareness_Campaign').in().as('events').count().next();\n" +
      "\n" +
      "def map = g.V().has('Metadata.Type','Object.Awareness_Campaign').in().as('events').groupCount().by('Event.Training.Status').next();\n" +
      "\n" +
