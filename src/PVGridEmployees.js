...
class PVGridEmployees extends PVGrid
{
  ...
  getSearchObj = (from, to, searchstr, searchExact, cols, extraSearch, sortcol, sortdir) =>
  {
    ...
    return {
      ...
      ".match(__.as('events').values('Event.Training_event.Status').as('event_status')" +
      ", __.as('events').id().as('event_id')" +
      ", __.as('employees').values('Person.Employee.Full_Name').as('Person.Employee.Full_Name')" +
      ", __.as('employees').values('Person.Employee.Title').as('Person.Employee.Title')" +
      ", __.as('employees').values('Person.Employee.Nationality').as('Person.Employee.Nationality')" +
      ", __.as('employees').id().as('emp_id')" +
      ")" +
      ".select('Person.Employee.Title','event_status','Person.Employee.Full_Name','Person.Employee.Nationality','emp_id', 'event_id')"
      , bindings: {
