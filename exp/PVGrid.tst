...
import PontusComponent from '../src/PontusComponent';
import { scaleDown2 as Menu } from '../src/PVBurgerMenu';
import PVGridColSelector from '../src/PVGridColSelector';
import PVGridReportButtonCellRenderer from '../src/PVGridReportButtonCellRenderer';
...
class PVGrid extends PontusComponent {
  ...
  private customFilter: string | undefined;
  private dataType: string | null;
  private extraSearch: any[] | undefined;
  ...
  constructor(props: Readonly<PVGridProps>) {
    ...
    this.subNamespace = props.subNamespace || '';
  ...
  ensureData = (fromReq, toReq) => {
    ...
    const fromPage = Math.floor(fromReq / this.PAGESIZE);
  ...
  componentWillUnmount = () => {
    ...
    this.props.glEventHub.off(
      `${this.namespace}${this.subNamespace ? this.subNamespace : ''}-pvgrid-on-col-settings-changed`,
